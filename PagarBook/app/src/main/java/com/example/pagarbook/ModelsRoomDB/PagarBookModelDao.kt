...
interface PagarBookModelDao {
    ...
    @Query("SELECT staffDetails.id, staffDetails.name, staffDetails.mobile,  SUM(staffattendance.dayAmount) as salary, staffDetails.salaryCycle  FROM  staffDetails LEFT JOIN staffattendance ON staffDetails.id = staffattendance.staffid GROUP BY staffDetails.id ")
